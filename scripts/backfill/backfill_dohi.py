...
from app.index_definitions.dohi_v01 import DAO_ENTITIES, DOHI_V01_DEFINITION
...
async def backfill_entity(entity: dict, days_back: int = 365):
    ...
    while current <= end_date:
        ...
        # path (dao_collector.score_dao → score_entity → aggregate()).
            ...
            execute(
                ...
                INSERT INTO generic_index_scores
                    (index_id, entity_slug, entity_name, overall_score,
                     raw_values, component_scores, category_scores,
                     formula_version, scored_date,
                ...
                """,
                (INDEX_ID, slug, name, result["overall_score"], raw_json,
                 json.dumps(result["component_scores"]),
                 json.dumps(result["category_scores"]),
                 FORMULA_VERSION, score_date, source,
                 ...
                 json.dumps(DOHI_V01_DEFINITION.get("aggregation", {}).get("params", {})),
