...
def compute_and_store_daily_rollup(target_date: date = None):
    ...
    try:
        ...
        # Report requests
        try:
            rr = fetch_one("SELECT COUNT(*) as c FROM api_request_log WHERE timestamp::date = %s AND (endpoint LIKE '/api/reports/%%' OR endpoint LIKE '/api/paid/report/%%')", (d,))
            report_requests = rr["c"] if rr else 0
        except Exception:
            report_requests = 0

        # x402 payments
        try:
            xr = fetch_one("SELECT COUNT(*) as c, COALESCE(SUM(price_usd), 0) as rev FROM payment_log WHERE timestamp::date = %s", (d,))
            x402_payments = xr["c"] if xr else 0
            x402_revenue = float(xr["rev"]) if xr else 0.0
        except Exception:
            x402_payments = 0
            x402_revenue = 0.0
        ...
        # State attestations
        try:
            sa = fetch_one("SELECT COUNT(*) as c FROM state_attestations WHERE cycle_timestamp::date = %s", (d,))
            state_attestations_count = sa["c"] if sa else 0
        except Exception:
            state_attestations_count = 0

        # Report attestations
        try:
            ra = fetch_one("SELECT COUNT(*) as c FROM report_attestations WHERE generated_at::date = %s", (d,))
            report_attestations_count = ra["c"] if ra else 0
        except Exception:
            report_attestations_count = 0

        # Query requests
        try:
            qr = fetch_one("SELECT COUNT(*) as c FROM api_request_log WHERE timestamp::date = %s AND endpoint = '/api/query'", (d,))
            query_requests = qr["c"] if qr else 0
        except Exception:
            query_requests = 0

        # CQI requests
        try:
            cr = fetch_one("SELECT COUNT(*) as c FROM api_request_log WHERE timestamp::date = %s AND endpoint LIKE '/api/compose/cqi%%'", (d,))
            cqi_requests = cr["c"] if cr else 0
        except Exception:
            cqi_requests = 0
        ...
        with get_conn() as conn:
            with conn.cursor() as cur:
                ...
                """, (d, total_requests, external_requests, internal_requests,
                      ...
                      state_attestations_count, report_attestations_count, query_requests, cqi_requests))
            conn.commit()
